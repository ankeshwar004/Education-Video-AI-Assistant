...
def create_session(session_id, video_id, title=None):
    ...
    return initialize_memory(session_id, video_id, title)
...
def list_sessions(video_id):
    
    video = get_video(video_id)
    if video is None:
        raise not_found("Video not found")
    return get_chat_sessions_for_video(video_id)
